...
from tg_bot.service.api_requests import create_or_update_clients_from_crm, find_user_in_crm, find_user_in_django, register_user_in_crm, register_user_in_db
...
async def handle_contact(message: Message):
    ...
    # Отправляем клавиатуру в зависимости от статуса
    if user_status == "2":  # Клиент
        await message.answer("Все готово к работе!\nВот мое главное меню:", reply_markup=main_menu_inline_keyboard_for_client)
    elif user_status == "1":  # Lead with group
        await message.answer(
        ...
        await message.answer(
            "Все готово к работе!⚡️\nВот мое главное меню:", reply_markup=main_menu_inline_keyboard_for_lead_with_group
        )
